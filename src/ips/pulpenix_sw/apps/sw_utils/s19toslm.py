...
l2_banks     = 1
l2_bank_size = 8192*2 # in words (32 bit)
if "64KI" in sys.argv:
    l2_bank_size = 16384*8
if "HAMSA" in sys.argv:
    l2_start     = 0x1ac00000
else:
    l2_start     = 0x00000000
...
###############################################################################
# write the stimuli to be stored as bytes in sflash
###############################################################################
# from words to bytes
l2_start   = l2_start   << 2
l2_end     = l2_end     << 2
...
l2_size    = l2_size    << 2  
tcdm_size  = tcdm_size  << 2  

sflash_start = 0x00000000
sflash_end   = 0x0003FFFF
...
s19_dict = {}
s19tosflash_dict = {}
s19toslm_f = 'build/test.s19'
s19tosflash_f = 'build/flash.s19'
s19_parse(s19toslm_f, s19_dict)
s19_parse(s19tosflash_f, s19tosflash_dict)


s19_iter = iter(sorted(s19tosflash_dict.keys()))
print("l2_size - %x, tcdm_size - %x" % (l2_size, tcdm_size))
for i in range(8):
    addr = next(s19_iter)
    if i < 4:  # imem size
        data = l2_size & 0xFF
        #s19tosflash_dict[addr] = data
...
for addr in sorted(s19tosflash_dict.keys()):
...
print " ********************* L2 *********************"
for addr in sorted(s19_dict.keys()):
    data = s19_dict[addr]
   # print("addr - %s,  data - %s" %( addr, data))
    # l2 address range
    if(addr >= l2_start and addr <= l2_end):
